...
import ckite.{CKiteClient, CKite}
import ckite.kvstore.{Get, Put}
  ...
  def apply(request: Request) = {
    request.method -> Path(request.path) match {
      ...
      case Method.Get -> Root / "kv" / key => {
        ...
        val get = Get(key)
        val result = if (localOption.getOrElse(false)) ScalaFuture.successful(ckite.asInstanceOf[CKiteClient].readLocal(get)) else ckite.read(get)
